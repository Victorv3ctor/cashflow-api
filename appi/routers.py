...
@app.post('/login')
def login_in(data: LoginRequest, service: AccountService = Depends(get_account_service)):
...
@app.get('/account')
def activity_status(service: TransactionService = Depends(get_transaction_service),
                        account=Depends(get_current_account)):
